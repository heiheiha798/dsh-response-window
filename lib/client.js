...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      return React.createElement('div', { className: 'drw-slide', 'data-running': running ? '1' : undefined },
        React.createElement('div', {
          ...
          React.createElement('span', { className: 'drw-head-title' },
            '响应 ' + turn + ' · ' + stats.total + ' 个工具调用' + (reasoning.length > 0 ? ' · ' + reasoning.length + ' 段思考' : '')
          ),
          running
    ...
    function ToolTurnSlide(props) {
      ...
      var EMPTY = { tools: [], reasoning: [] }
      var collected = useSession(function (snapshot) {
        ...
        var out = { tools: [], reasoning: [], seq: 0 }
        for (var i = 0; i < order.length; i++) {
          var candidate = chat.nodes.get(order[i])
          if (candidate === undefined) continue
          if (candidate.kind === 'tool-call') {
            out.tools.push(candidate)
            var s = candidate.seq
            if (typeof s === 'number' && s > out.seq) out.seq = s
          } else if (candidate.kind === 'assistant-step' && candidate.data && Array.isArray(candidate.data.blocks)) {
            var blocks = candidate.data.blocks
            for (var b = 0; b < blocks.length; b++) {
              var block = blocks[b]
              if (block && block.kind === 'reasoning' && typeof block.text === 'string' && block.text.trim() !== '') {
                out.reasoning.push(block.text)
              }
            }
          }
        }
    ...
    function hideNativeThink(root) {
      ...
      for (var r = 0; r < rows.length; r++) {
        ...
        for (var j = r - 1; j >= 0; j--) {
          var prev = rows[j]
          if (prev.matches && prev.matches('[data-chat-flow-kind="user"]')) break
          if (prev.querySelector && prev.querySelector('.drw-slide')) { hasSlide = true; break }
        }
        for (var k = r + 1; k < rows.length && !hasSlide; k++) {
          var next = rows[k]
          if (next.matches && next.matches('[data-chat-flow-kind="user"]')) break
