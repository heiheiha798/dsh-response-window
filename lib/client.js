  ...
  factory: (require) => {
    ...
    function injectStyles() {
      ...
      el.textContent = [
        ...
        '.drw-think { border-top: 1px solid var(--dsw-alias-border-strong, rgba(128,128,128,0.16)); flex: none; }',
        ...
        '.drw-think-icon { color: var(--dsw-alias-label-tertiary, #999); font-size: 12px; flex: none; }',
        '.drw-think-name { color: var(--dsw-alias-label-secondary, #bbb); font-size: 11px; font-weight: 600; letter-spacing: 0.04em; text-transform: uppercase; flex: none; }',
        '.drw-think-summary { min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--dsw-alias-label-tertiary, #999); font-size: 12px; flex: 1 1 auto; }',
        '.drw-think-toggle { color: var(--dsw-alias-label-tertiary, #888); font-size: 10px; flex: none; }',
        '.drw-think-body { margin: 0; padding: 2px 10px 8px 30px; color: var(--dsw-alias-label-secondary, #ccc); font-size: 12px; line-height: 1.6; white-space: pre-wrap; word-break: break-word; max-height: 240px; overflow-y: auto; }',
        ...
        '.drw-set-row { display: flex; align-items: center; justify-content: space-between; gap: 16px; width: 100%; }',
        '.drw-set-text { min-width: 0; }',
        '.drw-set-title { color: var(--dsw-alias-label-primary, #eee); font-size: 13px; font-weight: 500; }',
        '.drw-set-desc { color: var(--dsw-alias-label-secondary, #bbb); font-size: 12px; margin-top: 2px; }',
        ...
        '}',
        '.drw-set-live { font-size: 11px; color: var(--dsw-alias-label-tertiary, #888); white-space: nowrap; }',
      ].join('\n')
    ...
    function ThinkRow(props) {
      ...
      return React.createElement('div', { className: 'drw-think', 'data-open': open ? '1' : undefined },
        React.createElement('div', {
          ...
          React.createElement('span', { className: 'drw-think-icon', 'aria-hidden': true }, '\uD83D\uDCAD'),
          React.createElement('span', { className: 'drw-think-name' }, '思考 ' + (index + 1)),
    ...
    // ---- the per-turn slide -------------------------------------------------
    // One slide = one turn's implementation layer: reasoning (Think) blocks +
    // all tool calls, all inside the same bounded scroll window.
      ...
      return React.createElement('div', { className: 'drw-slide', 'data-running': running ? '1' : undefined },
        React.createElement('div', {
          ...
          React.createElement('span', { className: 'drw-head-title' },
            '响应 ' + turn
              + (segmentCount && segmentCount > 1 && typeof segmentIndex === 'number'
                  ? '·段' + (segmentIndex + 1) + '/' + segmentCount : '')
              + ' · ' + stats.total + ' 个工具调用'
              + (reasoning.length > 0 ? ' · ' + reasoning.length + ' 段思考' : '')
          ),
          running
        ...
        React.createElement('div', {
          ...
          reasoning.map(function (txt, i) {
            return React.createElement(ThinkRow, { key: 'think-' + i, text: txt, index: i })
          }),
          list.map(function (n) {
    ...
    function buildTurnSegments(order, chat) {
      ...
      function ensure() {
        if (current === null) {
          current = { tools: [], reasoning: [], startKey: null }
      ...
      for (var i = 0; i < order.length; i++) {
        ...
        if (n.kind === 'assistant-step') {
          ...
          // staged response (i.e. the segment that ends here).
          for (var b = 0; b < blocks.length; b++) {
            var block = blocks[b]
            if (block && block.kind === 'reasoning' && typeof block.text === 'string' && block.text.trim() !== '') {
              ensure().reasoning.push(block.text)
            }
          }
    ...
    function ToolTurnSlide(props) {
      ...
      var snapData = useSession(function (snapshot) {
      ...
      try {
        ...
        for (var i = 0; i < segments.length; i++) {
          if (segments[i].startKey === node.key) { segIndex = i; break }
        }
    ...
    function scanAssistantText(root, lines) {
      if (lines <= 0) return
    ...
    function hideNativeThink(root) {
      ...
      // Segment boundary in the DOM: a user row, or an assistant-step row that
      // carries visible staged text (a response shown to the user mid-turn).
      function isSegmentBoundary(row) {
        if (row.matches && row.matches('[data-chat-flow-kind="user"]')) return true
        if (!row.matches || !row.matches('[data-chat-flow-kind="assistant-step"]')) return false
        var md = row.querySelector('[class*="_markdown"]')
        return md !== null && md.textContent.trim() !== ''
      }
      for (var r = 0; r < rows.length; r++) {
        var row = rows[r]
    ...
    // applies the assistant text window + native-think hiding (session-scoped).
    function TextWindowDock(props) {
      var mountRef = React.useRef(null)
      var lines = useLiveLines() // live window size from Settings
      var linesRef = React.useRef(lines)
      ...
      React.useEffect(function () {
        ...
        var timer = null
        var scan = function () {
          try { scanAssistantText(root, linesRef.current) } catch (e) {}
          try { hideNativeThink(root) } catch (e) {}
        }
        scan()
        ...
        // Re-scan immediately when the window size changes.
        var resize = setTimeout(scan, 60)
        return function () {
          observer.disconnect()
    ...
    function ResponseWindowSettingsRow(props) {
      ...
      var resetLines = props.resetLines
      ...
      return React.createElement('div', { className: 'drw-set-row' },
        React.createElement('div', { className: 'drw-set-text' },
          ...
          React.createElement('div', { className: 'drw-set-desc' }, '每个响应 slide 的限高滚动窗口。0 = 不限高。改动即时生效。'),
        ),
        React.createElement('div', { className: 'drw-set-control' },
          React.createElement('span', { className: 'drw-set-live' }, '当前 ' + value + ' 行'),
          React.createElement('button', {
          ...
          React.createElement('button', {
            type: 'button', className: 'drw-set-btn', title: '重置为默认 10 行',
            onClick: resetLines,
          }, '重置'),
        ),
      )
    ...
    function apply(ctx, config) {
      ...
      // Durable settings binding: the host half registered the `dsh-response-window`
          ...
          inject: function () {
            return {
              ...
              setLines: function (value) {
                ...
                if (linesStore !== null && linesStore.getSnapshot() === n && n !== 0) {
                  // same value, no-op
                }
              ...
              resetLines: function () {
                if (settingsScope !== null) {
                  settingsScope.unset('lines').catch(function () {})
                }
                if (linesStore !== null) linesStore.set(currentConfig.lines)
              },
            }
          },
